...
def equalize_and_soften(vocab, words, eq_sets, bias_subspace, embedding_dim, l=0.2, verbose=True):
    ...
    valid_neutral_words = [w for w in words if w in vocab]

    # Notify if any words were skipped
    if len(valid_neutral_words) < len(words):
        skipped_words = set(words) - set(valid_neutral_words)
        print(f"Skipped words not in vocab: {skipped_words}")

    # Create tensors for neutrals and vocab
    neutrals_array = np.array([vocab[w] for w in valid_neutral_words])
    Neutrals = torch.tensor(neutrals_array).float().t()

    vocab_array = np.array(vocabVectors)  # Assuming vocabVectors already includes valid vectors
    Words = torch.tensor(vocab_array).float().t()
    ...
    for i, w in enumerate(Words.t()):
        transformedVec = torch.mm(Transform, w.reshape(-1, 1))
